...
static void DbgStr(LPCSTR lpszFormat, ...)
{
	va_list   args;
	int       nBuf;
	TCHAR     szBuffer[512];
...
BOOL CPlxReaderDlg::OnInitDialog()
{
	...
	m_buttonCheckSpike.SetCheck(0) ;
	m_buttonCheckEvent.SetCheck(0) ;
...
void CPlxReaderDlg::DumpHeaders()
{
	...
	for (iChannel = 0 ; iChannel < m_fileHeader.NumSlowChannels ; iChannel++)
	{
		...
		Put ("  Channel %d Name(%s): ADFreq %d Gain %d Enabled %d PreAmpGain %d SpikeChannel %d;", 
...
void CPlxReaderDlg::OnExtract() 
{
	...
	float bpss_2 = pow(2, m_fileHeader.BitsPerSlowSample) / 2;
	uint64_t count_datablock = 0;
	...
	for (iBlock = 0 ; ; iBlock++)
	{
			...
			(bShowSlow && (dataBlock.Type == PL_ADDataType)))
		{
			...
			LONGLONG timestamp = (static_cast<LONGLONG>(dataBlock.UpperByteOf5ByteTimestamp) << 32)
			...
			{
				DbgStr("ET %d", count_datablock);
				DbgStr(" %.2f", timestamp / 30000.0);
				DbgStr(" %ld", timestamp);
				DbgStr(" %d", dataBlock.Channel);
				DbgStr(" %d", nWords);
				DbgStr(": %d", dataBlock.Unit);
				DbgStr("\n");
				continue;
			...
			else if (dataBlock.Type == PL_ADDataType)
			{
				...
				DbgStr(" %d", dataBlock.Channel);
				DbgStr(" %d", nWords);
				DbgStr(": %d", m_slowChannels[dataBlock.Channel].SpikeChannel);
				DbgStr(" %.4f", v);
				DbgStr("\n");
				continue;
